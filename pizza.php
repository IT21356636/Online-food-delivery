...
<section class="menu1" id="menu1">
	 <h1 class="heading1"> <span>Hot</span> & <span>spicy</span>Pizza</h1>
	 <div class="box-container">
		 		 
        <?php
            $query = "select * from food where image like 'p%'";
            $result = mysqli_query($connection,$query);
            if(mysqli_num_rows($result)>0){
                while($row = mysqli_fetch_array($result)){
                    ?>
                    <div class="box">						
                        <form method="post" action="pizza.php?action=add&id=<?php echo $row["id"];?>">
                            <div class="pizza">
								<div class="image">
